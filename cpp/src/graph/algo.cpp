...
get_in_out_vals(const transaction_graph::internal_graph_t &graph) {
...
get_stakes(const transaction_graph::internal_graph_t &graph) {

  std::unordered_map<std::string, double> ret;

  std::unordered_map<std::string, in_out_val> in_out_vals = get_in_out_vals(graph);
  for (auto it = in_out_vals.begin(); it != in_out_vals.end(); it++) {
    ret.insert(std::make_pair(it->first, it->second.in_val-it->second.out_val));
  }
  return ret;
}
...
get_degree_sum(const transaction_graph::internal_graph_t &graph) {

  std::unordered_map<std::string, int> ret;

  std::unordered_map<std::string, in_out_degree> in_out_degrees = get_in_out_degrees(graph);
  for (auto it = in_out_degrees.begin(); it != in_out_degrees.end(); it++) {
    ret.insert(std::make_pair(it->first, it->second.in_degree+it->second.out_degree));
  }
  return ret;
}
...
transaction_graph_ptr merge_two_graphs(transaction_graph_ptr tg,
  ...
  for (boost::tie(vi, vi_end) = boost::vertices(sgi); vi != vi_end; vi++) {
    oeiterator_t oei, oei_end;
    for (boost::tie(oei, oei_end) = boost::out_edges(*vi, sgi);
         oei != oei_end; oei++) {
